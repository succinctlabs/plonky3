...
mod tests {
    ...
    use crate::ZerofierOnCoset;

    #[test]
    fn test_lagrange_basis_unnormalized() {
        type F = BabyBear;

        let log_n = 3;
        let rate_bits = 2;
        let log_coset_size = log_n + rate_bits;
        let n = 1 << log_n;
        let coset_size = 1 << log_coset_size;

        let g_h = F::primitive_root_of_unity(log_n);
        let g_k = F::primitive_root_of_unity(log_n + rate_bits);
        let coset_shift = F::multiplicative_group_generator();
        let subgroup_last = g_h.inverse();

        let zerofier = ZerofierOnCoset::new(log_n, rate_bits, coset_shift);
        let l_first = zerofier.lagrange_basis_unnormalized(0);
        let l_last = zerofier.lagrange_basis_unnormalized(n - 1);

        let coset = cyclic_subgroup_coset_known_order(g_k, coset_shift, coset_size).collect_vec();
        for (x, l_first_x, l_last_x) in izip!(coset, l_first, l_last) {
            let z_h = x.exp_power_of_2(log_n) - F::ONE;
            assert_eq!(l_first_x, z_h / (x - F::ONE));
            assert_eq!(l_last_x, z_h / (x - subgroup_last));
        }
    }
}
